  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {statConfig.map((stat) => {
        ...
        const color    = typeof stat.color === "function" ? stat.color(value as boolean) : stat.color;
        const formatted = value !== undefined ? stat.format(value as never) : "—";
        ...
        return (
          <div
            ...
            style={{ ...GLASS_BASE }}
          >
            <p className="font-sans text-[13px] mb-2" style={{ color: "var(--text-muted)" }}>
              {stat.label}
            </p>
            {isLoading ? (
              <Skeleton className="h-7 w-28 mt-1" />
            ) : (
              <p className="font-mono font-medium text-[20px]" style={{ color }}>
                {formatted}
              </p>
            )}
          </div>
